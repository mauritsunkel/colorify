...
colorify <- function(
  ...
  if (nn == Inf && length(colors) > n) {
    colors <- colors[seq_along(colors)]
  ...
  if (is.numeric(colors_lock)) {
    ...
    if (nn[1] > n && nn[1] %% length(colors_lock) == 0) {
      ...
      colors_lock_i <- colors_lock[seq_along(colors)]
    } else {
  ...
  ## plot colors
  if (!isFALSE(plot)) {
    if (isTRUE(plot)) graphics::pie(rep(1, length(colors)), labels = if (grepl("l|L", plot)) seq_along(colors) else NA, col = colors, border = NA)
    if (grepl("i|I", plot)) graphics::image(seq_along(colors), 1, as.matrix(seq_along(colors)), col = colors, xlab = "", ylab = "", xaxt = "n", yaxt = "n", bty = "n")
    if (grepl("i|I", plot) && grepl("l|L", plot)) graphics::text(seq_along(colors), rep(1, length(colors)), labels = seq_along(colors), col = "black")
  }
  ## export colors
